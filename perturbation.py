...
parser.add_argument('--universal_train_portion', default=0.2, type=float) #changed to 0.8 from 0.2
    ...
    for i, (images, labels) in enumerate(data_loader[eval_target]):
        ...
        if random_noise is not None:
            for i, (image, label) in enumerate(zip(images, labels)):
                ...
                #images[i] = transforms.Grayscale(3)(images[i])+sample_noise #grayscale
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    if (args.train_data_type == 'ImageNetMini') and args.perturb_type == 'samplewise':
    #if (args.train_data_type == 'ImageNetMini' or args.train_data_type=='MCOCO') and args.perturb_type == 'samplewise':
        ...
        print("i am here")
        #print(len(data_loader['train_dataset'].dataset))
    #if (args.train_data_type == 'MCOCO') and args.perturb_type == 'samplewise':
    #if (args.train_data_type == 'ImageNetMini' or args.train_data_type=='MCOCO') and args.perturb_type == 'samplewise':
     #   data_loader = datasets_generator._split_validation_set(0.6, train_shuffle=False, train_drop_last=False)
      #  data_loader['train_dataset'] = data_loader['train_subset']
       # print("i am here")
        #print(len(data_loader['train_dataset'].dataset))
    else:
        ...
        if args.attack_type == 'min-min' and not args.load_model:
            ...
            for j in tqdm(range(0, args.train_step), total=args.train_step):
                ...
                for i, (image, label) in enumerate(zip(images, labels)):
                    ...
                    sample_noise = torch.from_numpy(mask).to(device)
                    #images[i] = transforms.Grayscale(3)(images[i])+sample_noise #grayscale
                    images[i] = images[i] + sample_noise #color channels
                    train_idx += 1
