        ...
        for _ in range(self.num_steps):
            ...
            if isinstance(criterion, torch.nn.CrossEntropyLoss):
                if hasattr(model, 'classify'):
                    model.classify = True
                # for gray scale
                #logits = model(transforms.Grayscale(3)(perturb_img))
            ...
            loss.backward()
            # for grayscale
            #eta = self.step_size * perturb_img.grad.data.sign().sum(axis=1)*(-1)
            #eta = eta.unsqueeze(1)
            #perturb_img = Variable(perturb_img.data + eta, requires_grad=True)
            #eta = transforms.Grayscale(1)(torch.clamp(perturb_img.data - images.data, -self.epsilon, self.epsilon))
